...
OWN_DOCS = ('docs/CLAUDE_CHAMPION_CLASS_20260916.md',)
...
def main():
    ...
    args = ap.parse_args()
    if args.phase == 'before' and (CC.OUT / 'integrity' / 'snapshot_before.json').exists():
    ...
    snap = dict(phase=args.phase, taken_at=time.strftime('%Y-%m-%d %H:%M:%S'),
                sha256_incremental_q=S.sha_tree(CC.IQ), sha256_cohort_role=S.sha_tree(CC.CR),
                sha256_full_corpus_read_targets=S.fc_read_targets(), inventory_full_corpus=S.inventory(CC.FC),
                sha256_parent_scripts={p.name: C.sha256_file(p) for p in sorted((C.ROOT / 'scripts').glob('*.py')) if not p.name.startswith('cc20260916_')},
                sha256_parent_tests={p.name: C.sha256_file(p) for p in sorted((C.ROOT / 'tests').glob('*.py')) if not p.name.startswith('test_cc20260916_')},
                ...
                sha256_ddragon={p.relative_to(CC.OUT).as_posix(): C.sha256_file(p) for p in sorted(CC.DDRAGON.rglob('*.json'))})
    snap['seconds'] = round(time.time() - t0, 1)
    snap['counts'] = {k: len(v) for k, v in snap.items() if isinstance(v, dict)}
    sha = C.write_json(CC.OUT / 'integrity' / f'snapshot_{args.phase}.json', snap)
    print(f'{args.phase}: counts {snap["counts"]} in {snap["seconds"]} s sha256 {sha}')
    if args.phase in ('after', 'final'):
        before = C.read_json(CC.OUT / 'integrity' / 'snapshot_before.json')
        diff = {}
        ...
        C.write_json(CC.OUT / 'integrity' / ('snapshot_diff.json' if args.phase == 'after' else 'snapshot_final_diff.json'), diff)
