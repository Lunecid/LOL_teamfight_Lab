...
TESTS = ['tests/test_hs20260916_contracts.py']


def main():
    HS.log_command()
    out = HS.OUT / 'contract_tests'
    ...
    t0 = time.time()
    proc = subprocess.run(cmd, cwd=str(HS.ROOT), capture_output=True, text=True, encoding='utf-8', errors='replace')
    log = proc.stdout + '\n' + proc.stderr
    ...
    res = dict(run=run, command=' '.join(cmd[1:]), returncode=proc.returncode, passed_count=int(m.group(1)) if m else 0,
               ...
               test_sha256={t: C.sha256_file(HS.ROOT / t) for t in TESTS}, source_sha256=HS.own_source_hashes(), log=f'contract_tests/run{run}.txt')
    C.write_json(out / f'result_run{run}.json', res)
    if res['passed']:
        C.write_json(out / 'result.json', res)
    else:
        HS.log_failure('contract_tests', f'run {run} failed', returncode=proc.returncode, tail=log[-3000:])
