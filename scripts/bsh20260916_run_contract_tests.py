...
TESTS = ['tests/test_bsh20260916_contracts.py']


def main():
    B.log_command()
    out = B.OUT / 'contract_tests'
    ...
    t0 = time.time()
    proc = subprocess.run(cmd, cwd=str(B.ROOT), capture_output=True, text=True, encoding='utf-8', errors='replace')
    log = proc.stdout + '\n' + proc.stderr
    ...
    res = dict(run=run, command=' '.join(cmd[1:]), returncode=proc.returncode, passed_count=int(m.group(1)) if m else 0, failed_count=int(f.group(1)) if f else 0,
               ...
               seconds=round(time.time() - t0, 1), at=time.strftime('%Y-%m-%d %H:%M:%S'), test_sha256={t: C.sha256_file(B.ROOT / t) for t in TESTS},
               source_sha256=B.own_source_hashes(), log=f'contract_tests/run{run}.txt')
    C.write_json(out / f'result_run{run}.json', res)
    if res['passed']:
        C.write_json(out / 'result.json', res)
    else:
        B.log_failure('contract_tests', f'run {run} failed', returncode=proc.returncode, tail=log[-3000:])
