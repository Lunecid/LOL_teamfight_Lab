#!/usr/bin/env python3
"""Shared forecast diagnostics for review-response (CORP-style + triad).

CORP Brier decomposition (Dimitriadis–Gneiting–Jordan 2021 style):
  fit isotonic recalibration x* = iso(p) on the *evaluation* sample for DIAGNOSIS only;
  UNC = weighted mean (y - ȳ)^2
  DSC = weighted mean (x* - ȳ)^2
  MCB = weighted mean (p - x*)^2
  BS  = MCB - DSC + UNC   (equals weighted Brier up to numerical tolerance)
...
def corp_brier_decomposition(y: np.ndarray, p: np.ndarray, w: Optional[np.ndarray] = None) -> Dict[str, Any]:
    """CORP-style Brier score decomposition via isotonic recalibration (diagnostic)."""
    ...
    unc = weighted_mean((y - ybar) ** 2, w)
    # Isotonic: nondecreasing map p -> E[y|p]
    iso = IsotonicRegression(y_min=0.0, y_max=1.0, increasing=True, out_of_bounds="clip")
    # sample_weight required for match-weighted CORP; fail closed if unsupported
    iso.fit(p, y, sample_weight=w)
    p_star = np.clip(iso.predict(p), 0.0, 1.0)
    dsc = weighted_mean((p_star - ybar) ** 2, w)
    mcb = weighted_mean((p - p_star) ** 2, w)
    recon = mcb - dsc + unc
    # reliability curve knots from isotonic
    # unique fitted levels
    order = np.argsort(p)
    p_sorted = p[order]
    ...
    return dict(
        ...
        reconstruction_gap=float(bs - recon),
        mean_y=ybar,
        mean_p=weighted_mean(p, w),
        reliability_curve=dict(p_hat=xs, p_star=ys, n=ns),
        note="CORP isotonic decomposition on this evaluation sample — diagnostic only; do not use as frozen-model fix.",
    )
