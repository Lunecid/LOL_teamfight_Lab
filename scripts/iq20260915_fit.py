...
def assert_unique_keys(g, s_ms):
    """Pre-fit guard (added post-run, code only): every loaded row key (match, s_ms) must be unique."""
...
def partial_family_artifacts(base, fam, coh):
    """Existing artifacts of an unfinished (family, cohort) fit that a rerun would overwrite (added post-run, code only)."""
    base = Path(base)
    found = sorted(p for p in (base / 'models' / coh / fam).glob('*.joblib')) if (base / 'models' / coh / fam).exists() else []
    others = [base / 'predictions' / f'{fam}_{coh}_trainval.npz'] + ([base / 'internal_stop' / f'lgbm_{coh}.json'] if fam == 'lgbm' else [])
    ...
    try:
        ...
        if partial:
            raise SystemExit(f'partial-family artifacts exist without {sel_path.name} and would be overwritten: {partial}. '
                             'Retained untouched; inspect them and move them to a named attempt directory before rerunning this family/cohort.')
        ...
        sel = dict(
            ...
            lgbm_seed_metrics_DESCRIPTIVE=seed_met, split_counts=counts, pre_fit_unique_row_keys=n_unique_keys, weights=wrec,
