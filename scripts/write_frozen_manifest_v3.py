...
def main():
    ...
    manifest = {
        ...
        'base_commit': head,
        'workspace': str(ROOT),
        'label_contract': {'label': 'sign(V3(S_end) - V3(S_pre))', 'boundary': 'end = last cluster kill',
                           'weight': '|delta-V_end| x match weight', 'comparison': 'y_market_event on the same rows'},
