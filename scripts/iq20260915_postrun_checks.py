...
def bootstrap_cell_acceptance(bt, y, g):
    ...
    Eligible cell (>= 30 distinct matches and both Y classes): a computed record with exactly 1000 replicates, seed
    20260915, matches/rows equal to the cell, model_ci for the five bootstrap models, the five planned contrasts in
    protocol order, and finite ordered 95% intervals with finite replicate counts = replicates for Brier / log loss and
    replicates - degenerate for AUC (0 <= degenerate < replicates). Ineligible cell: explicit computed=False with reason.
    """
    y = np.asarray(y).astype(int)
    g = np.asarray(g).astype(str)
    nm = int(len(np.unique(g)))
    classes = int(len(np.unique(y))) if len(y) else 0
    eligible = nm >= Q.SPARSE_MATCHES and classes == 2
    if not eligible:
    ...
    if reps != Q.BOOT_REPS:
        P.append(f'replicates {reps} != {Q.BOOT_REPS}')
    if bt.get('seed') != Q.BOOT_SEED:
        P.append(f'seed {bt.get("seed")} != {Q.BOOT_SEED}')
    ...
    if set(bt.get('model_ci', {})) != set(BOOT_MODELS):
        P.append(f'model_ci models {sorted(bt.get("model_ci", {}))} != {sorted(BOOT_MODELS)}')
    got = [(p.get('a'), p.get('b'), p.get('label')) for p in bt.get('pairs', [])]
    if got != [tuple(c) for c in Q.CONTRASTS]:
        P.append(f'contrasts {got} != planned five')
...
def main():
    ...
    ap.add_argument('--output', default=str(Q.OUT / 'validation.json'), help='validation file (candidate runs may write elsewhere first)')
    args = ap.parse_args()
    Q.log_command()
    ...
    try:
        ...
        # historical prerequisite = latest PASSING run before the first full fit (post-run audit tests live in audit_followup/ and never count)
        ct = max((r for r in runs if r['passed'] and ts(r['at']) <= first_fit), key=lambda r: ts(r['at']))
        add('order_snapshot_tests_protocol_smoke_latest_tests_before_full_fits',
            ...
            protocol=proto['written_at'], smoke_frozen=sfz['frozen_at'], latest_passing_tests_before_full_fits=ct['at'], prerequisite_run=ct['run'],
        ...
        add('bootstrap_acceptance_all_B40_B45_cells', not boot_problems and sum(boot_counts.values()) == len(Q.EVAL_SETS) * len(Q.COHORTS) * len(BOOT_CELLS),
            counts=boot_counts, problems=boot_problems[:50],
            rule='eligible (>=30 matches, 2 classes): 1000 reps, seed 20260915, 5 planned contrasts, finite ordered CI, finite replicates '
        ...
        sha = C.write_json(Path(args.output), out)
