...
## 2026-09-21 추가 — 규모 분리 승격(C, 본문 2-코호트)이 RQ에 미치는 영향
- 저자가 S(2–3인) 코호트를 저널 본문 주 표에 넣기로 결정했다(SCALE_SPLIT 계약 §8). RQ 문구는 여전히 미결이며 이 결정으로 자동 변경되지 않는다.
- 어느 안(A/B/C)을 택하든 **J-RQ3의 범위 목록에 '교전 규모(T·S 코호트)'를 넣어야** 본문 구조와 맞는다. J-RQ2는 두 코호트 각각에 대해 답한다(코호트별 주 대비 1개).
- 금지: 코호트 간 절대 성능 비교를 RQ 답으로 쓰는 것("어느 교전이 더 예측 가능한가"). 허용: "각 코호트 안에서 PT_flex를 넘는 이득이 존재하는가, 그 이득이 코호트 사이에 전이되는가(부분), 합동 학습이 이득을 주는가(아니오)".

## 2026-09-21 검토자 권고 (저자 전달; 결정 아님) — 안 C 역할 배분 예시
| 질문 | 맡길 역할 |
|---|---|
| RQ1 | 정의한 승률 변화·방향이 무엇을 반영하며, 정의와 평가기 변경에 얼마나 안정적인가? |
| RQ2 | 각 코호트에서 교전 전 상태가 초기 승률·시간의 유연한 기준선을 넘어 추가 예측 이득을 제공하는가? |
| RQ3 | 그 이득은 균형 상태, 작은 변화 제외, 교전 규모별 적용·전이, 다른 패치·지역에서 어디까지 관측되는가? |

- 이 표는 검토자의 권고이며 저장소의 저자 결정이나 잠금 문구를 바꾸지 않는다. 채택 시 `JOURNAL_RESEARCH_PLAN` §6·`J_RQ1_SCOPE_LOCK` §4·`COG_SUCCESSION_LOCK` §5에 날짜 표기 개정 이력을 추가한다.
- 함께 확인된 사항: logit(352)/LightGBM(362)은 동일 입력이 아니므로 "동일 정보에서의 학습기 선택" 절은 답할 수 없고, 수행하지 않은 정보집합 절제의 답으로도 쓰지 않는다.
