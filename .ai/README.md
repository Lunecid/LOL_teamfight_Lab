...
## 실행 순서와 의존 (2026-09-20 밤 갱신)
- **완료:** T001–T004 (Claude 세션이 직접 실행, 보고서 `.ai/reports/`).
- **실험선 (Cursor 실행):** T007 (코호트 매개변수화 + T 회귀) → T008 (S 라벨 + manifest) → T009 (적합·평가·보고). 계약 `docs/SCALE_SPLIT_EXPERIMENT_CONTRACT_20260920.md`를 먼저 읽는다. 각 단계 보고서 후 Claude 검토.
- **문서선 (병행 가능):** T005 (계보 사본 + Methods `\pending` 3건 해소), T006 (상수표) — 작업서 작성 대기.
- 원래 표: 
