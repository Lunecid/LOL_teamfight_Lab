...
\input{tables/tab_primary_S}

각 코호트 안에서 교전 전 상태 모델은 시험된 $(\ppre, t)$의 유연한 함수보다 적정 점수 손실을 낮춘다. 두 블록은 행 수(\numSTest{} 대 \numTTest), 경기 가중 양성 비율(S \posRateS, T \posRateT), $\Bforty$ 비율(\shareSBforty\% 대 \shareTBforty\%)이 다르므로, 각 블록의 절대 Brier와 AUC는 그 코호트 안에서 읽는다.
