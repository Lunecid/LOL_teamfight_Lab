...
OUT_DIMS = ROOT / "thesis/latex/tables/gen/tab_input_dims.tex"
OUT_LIST = ROOT / "thesis/latex/tables/gen/tab_feature_list.tex"
CHECK_ONLY = False


def tt(name):
    """LaTeX \\texttt{} for a column name (underscores escaped)."""
...
def main() -> int:
    ...
    # ------------------------------------------------- per-model input widths
    n_F = len(groups["player_snapshot"])
    n_E = (len(groups["player_event"]) + len(groups["team_event"])
           + len(groups["team_time_interaction"]))
    dim_rows = [
        (f"평가기 $\\Vhat$ (\\path{{A_MLP_expanded}})",
         f"수치 {n_numeric} + 챔피언 ID {n_champ}", exp_w),
        (f"예측기 $q$ (\\path{{logit_state}})",
         f"수치 {n_numeric} + $\\ppre$", n_numeric + 1),
        (f"진단용 LightGBM (\\path{{lgbm_state}})",
         f"수치 {n_numeric} + 챔피언 ID {n_champ} + $\\ppre$", exp_w + 1),
    ]
    arm_desc = {
        "M-F0": f"$C$ + $Q_0$ + $\\ppre$",
        "M-F1": f"M-F0 + 참가자 상태 {n_F}",
        "M-F2": f"M-F0 + 사건 {n_E}",
        "M-F3": f"M-F0 + 참가자 상태 {n_F} + 사건 {n_E}",
    }
    for a, spec in arms.items():
        dim_rows.append((f"정보군 arm {a}", arm_desc[a], spec["expected_dim"]))
    dims_tex = [
        "%% GENERATED by thesis/tools/gen_feature_census.py -- do not edit by hand.",
        "\\begin{table}[!t]",
        "  \\centering",
    ...
    for i, (who, what, n) in enumerate(dim_rows):
        if i == 3:
            dims_tex.append("  \\midrule")
        dims_tex.append(f"    {who} & {what} & {n} \\\\")
    dims_tex += [
        "  \\bottomrule",
        "  \\end{tabularx}",
        "  \\tabnote{수치 " + str(n_numeric) + "열의 블록 구성은 표 \\ref{tab:input-blocks}, 열 이름은 표 "
        "\\ref{tab:app-feature-list}에 있다. 두 열은 어느 입력에도 들어가지 않는다. "
        f"\\texttt{{snapshot\\_age\\_s}}는 감사 메타데이터로 기록되어 원 폭 {raw_w}열에서 빠지고, "
        f"챔피언 ID {n_champ}열은 $q$와 정보군 arm에서 빠진다. "
        "$C = (\\ppre,\\ t,\\ t^2)$, $Q_0$ = 자료 품질 1열. "
        "정보군 arm은 LightGBM 아래의 비교이므로 절대 Brier가 $q$와 같은 척도 위에 있지 않다.}",
        "\\end{table}",
        "",
    ]

    # -------------------------------------------- the generating field names
    def joined(names):
        return ", ".join(tt(n) for n in names)

    list_rows = [
        ("경기 시각", f"1 $\\times$ {len(groups['clock'])} = {len(groups['clock'])}", joined(groups["clock"])),
        ("자료 품질", f"1 $\\times$ {len(groups['quality'])} = {len(groups['quality'])}", joined(groups["quality"])),
        ("참가자 상태",
         f"{n_slots_snap} $\\times$ {len(snap_fields)} = {len(groups['player_snapshot'])}",
         "\\texttt{participant\\_slot}$i$\\texttt{\\_}$\\ast$, $i = 0, \\ldots, "
         f"{n_slots_snap - 1}$: " + joined(snap_fields)),
        ("참가자 사건",
         f"{n_slots_ev} $\\times$ {len(ev_fields)} = {len(groups['player_event'])}",
         "같은 슬롯 접두: " + joined(ev_fields)),
        ("팀 사건",
         f"{n_sides} $\\times$ {len(team_fields)} = {len(groups['team_event'])}",
         "\\texttt{blue\\_}$\\ast$, \\texttt{red\\_}$\\ast$: " + joined(sorted(team_fields))),
        ("팀 사건 $\\times$ 시각",
         f"{n_sides} $\\times$ {len(team_fields)} = {len(groups['team_time_interaction'])}",
         f"위 {len(team_fields)}개 이름에 \\texttt{{\\_x\\_time}}을 붙인 열; 값은 부모 $\\times$ "
         f"\\texttt{{query\\_ms}}$/${denom_ms // 60000}분"),
        ("챔피언 ID",
         f"{n_slots_snap} $\\times$ 1 = {len(groups['champion'])}",
         "\\texttt{participant\\_slot}$i$\\texttt{\\_champion\\_id}; $q$에서 제외"),
    ]
    n_names_distinct = (len(groups["clock"]) + len(groups["quality"]) + len(snap_fields)
                        + len(ev_fields) + 2 * len(team_fields) + 1)
    list_tex = [
        "%% GENERATED by thesis/tools/gen_feature_census.py -- do not edit by hand.",
        "\\begin{table}[H]",
        "  \\centering",
    ...
    for who, shape_s, names in list_rows:
        list_tex.append(f"    {who} & {shape_s} & {names} \\\\")
        list_tex.append("  \\addlinespace")
    list_tex = list_tex[:-1]
    list_tex += [
        "  \\bottomrule",
        "  \\end{tabularx}",
        "  \\tabnote{슬롯 $i$는 역할 슬롯이며 진영 접두 "
        "\\texttt{blue}\\,/\\,\\texttt{red}는 팀을 가른다. 각 슬롯과 각 진영이 같은 이름 집합을 갖는다. "
        f"서로 다른 이름 {n_names_distinct}개가 열 {exp_w}개를 생성한다. "
        f"수치 열의 합은 {n_numeric}, 챔피언 ID를 더한 합은 {exp_w}이다. "
        f"\\texttt{{snapshot\\_age\\_s}}는 감사 메타데이터이며 이 표에 없다. "
        "전수 대조는 \\path{thesis/docs/FEATURE_CENSUS.md}.}",
        "\\end{table}",
        "",
    ]
    ...
    if not CHECK_ONLY:
        ...
        OUT_DIMS.write_text("\n".join(dims_tex), encoding="utf-8")
        OUT_LIST.write_text("\n".join(list_tex), encoding="utf-8")
    ...
    print("wrote " + ", ".join(str(x.relative_to(ROOT)) for x in (OUT_MD, OUT_TEX, OUT_DIMS, OUT_LIST)))
