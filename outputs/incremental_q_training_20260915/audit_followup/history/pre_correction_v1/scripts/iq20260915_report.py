...
def main():
    ...
    w('- 절대 개선 폭은 Brier 소수 셋째~넷째 자리 수준이다. B40/B45는 p_pre가 .5 근처인 모집단이라 기저 Brier가 0.25에 가깝고, 구성·유병률·시간대가 달라 전체와의 차이를 '
      '"어려움의 인과효과"로 해석하지 않는다. p_pre .5는 V 모델의 추정 균형이지 실제 승률의 정답이 아니다.')
    ...
    rows = [
        ...
        ('p_pre', 'V(S_pre), TRAIN은 자기 경기 제외 OOF fold adapter, 그 외 final V', 'labels npz adapter_id/sha256; selection *.oof_provenance', '상속·검증'),
