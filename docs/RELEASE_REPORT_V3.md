...
| 항목 | 내용 |
|---|---|
| 문서 | Release Report / Patch Notes, 정의 v3 · 코퍼스 v3.2 |
| 기간 | 2026-09-07 ~ 2026-09-09 |
| 브랜치 | `feature/fight-boundary-pipeline` (986b564 → aac7131, 19 커밋) |
| 상태 | 정의·라벨·입력 확정, 외부 감사 대응 완료, 코퍼스 v3.3 구축 중 |
...
| 6 | 아이템 해시 범주형 오지정 | 제외 | — |
| 7 | 회귀 가격 해석 | 표기 수정 + 경기 단위 부트스트랩 | 재실행 |
...
*(구축 완료 후 기입: market_event(가격) / 범주형 변형 / market_lex / Eq.3 각각의 전체·규모별 AUC와
v3 대비 차이. 이 차이가 누수 2건·결함 2건·공간 블록 복원·가격표의 순효과다.)*
...
1. v3.2 결과 확정 → spec·config 기본값(G, D, R, B, H, LABEL_TYPE)을 v3.2로 갱신, 논문 초안 수치 갱신.
