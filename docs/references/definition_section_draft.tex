...
\cite{silverman1981multimodality}. Across 9,903 matches (497,575 intervals) the valley is
at 13.3\,s (match-level bootstrap 95\% CI 11.8--15.7), stable under a four-fold change of
bandwidth (12.6--13.6\,s), across the three patches (13.0--14.1\,s), and across match
intensity terciles (11.4--14.5\,s). The mixture crossing gives 14.3\,s. Clustering
agreement (adjusted Rand index $\geq 0.9$) holds for $G \in [10, 18]$\,s, so the value is
reported together with this plateau, in the spirit of stability-based cluster selection
...
4,263 units (95\% CI 4,224--4,302) for pairs within $G$; it lies between 4,351 and
4,921 units in every map region (lanes, river, jungle, both pits, bases), so a single
$D$ suffices. Pairs within 7\,s but farther than 4,000 units share a champion only 3--5\%
of the time: these are simultaneous fights elsewhere. The sharing rate is 93.6\% for pairs
the rule links and 10.9\% for the simultaneous-but-distant pairs it separates.
...
players on a side \cite{ke2022}. $R = 1{,}800$ units is a mechanics anchor: 89.3\% of the
673 ability casts in the patch's Data Dragon fall within it, the same construction by which
Schubert et al.\ set their combat range at the distance covering about 85\% of observed
damage \cite{schubert2016encounter}. $R$ cannot be estimated from Match-V5 because the
positions of non-participating champions at kill time are not recorded; presence is
therefore an estimate from interpolated 60\,s frames, and we report its sensitivity: moving
the anchor to the episode's kill centroid (median shift 663 units) changes the verdict for
6\% of episodes with no directional bias in presence counts or scale classes. $B$ has no
data-driven estimate and is reported as a fixed lead, not as a fight onset.
...
$G = 13.9, 12.6, 13.9$\,s and $D = 4{,}309, 4{,}146, 4{,}296$ units, all inside, and one
definition serves the corpus. This mirrors the stability analysis of Zaliapin and
