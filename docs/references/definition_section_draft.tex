...
An episode becomes an \emph{engagement} when, at the cutoff $\tau$ placed $B=10$\,s before
...
whose positions are interpolated between 60\,s frames, so both are set inside brackets
defined by the game's own rules. For $R$ the game offers three proximity constants:
champions see each other within 1,350 units, a dying champion shares experience with
allies within 1,600 units, and the longest non-ultimate engage ability (Zac's Elastic
Slingshot at rank five) reaches 1,800 units; beyond that only semi-global ultimates such
as Ornn's (2,500 units) close the distance. We use the upper end, $R = 1{,}800$, the same
logic by which Schubert et al.\ set their combat range at the distance covering most
observed damage \cite{schubert2016encounter}. For $B$ the game credits a kill or an
assist to every champion who contributed within the preceding 15\,s and drops a champion
out of combat 5\,s after the last exchange; OpenDota's team-fight label, used by Tot et
al.\ \cite{tot2021camera}, likewise starts 15\,s before the first death. We use
$B = 10$\,s inside that bracket and call it a lead, not a fight onset. Because the gate
is a convention inside a bracket, we report its leverage. On 81,914 kill episodes from
2,971 matches, the share that qualifies as an engagement is 19.1\% at (1,800 units,
10\,s); it is 14.7\% at 1,600 units, 9.6\% at 1,350 and 34.2\% at 2,500, and 26.5\% at
5\,s and 13.8\% at 15\,s, monotone in both constants; the fully rule-anchored point
(1,600 units, 15\,s) keeps 54\% of the reference set. The presence gate, not $G$ or $D$,
is therefore the constant that decides how many engagements a match contains, so we
re-ran the tabular baseline at the rule-anchored point with every other constant fixed:
the corpus shrinks from 994,365 to 531,984 engagements and the held-out test AUC moves
from 0.669 (95\% CI 0.665--0.672) to 0.660 (0.656--0.663), while on the 32,820 test
engagements that both gates accept the two models are indistinguishable (0.659 vs 0.658)
and the engagements only the reference gate accepts are the more predictable ones
(0.673). The gate changes how many engagements there are, not how predictable they are.
...
pooled plateau, its $D$ within 10\% of the pooled value with overlapping intervals, and its
$R$ coverage within two points. For patches 15.14--15.16 the per-patch values are
...
intervals and the ARI plateau; predictive validity -- the engagement label is predictable
at AUC 0.746 from pre-cutoff state (Section~\ref{sec:results}). Construct validity is a
