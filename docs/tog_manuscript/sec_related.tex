...
%     subsec:future-work).  sec_limit.tex is not written yet, so its label is rendered as
%     \pending{ref}{sec:limit}, the convention of sec_intro.tex.
...
%     calibration";
%   - \ref{sec:limit} -> \pending{ref}{sec:limit}; prose shortened from about 1.1 pages to one page.
%
...
(Section~\pending{ref}{sec:limit}). Whether they help needs a user study
...
still be poorly calibrated, which post-processing by histogram binning, isotonic regression or a
...
cutoffs, which the kills that follow select; the value model of Section~\pending{ref}{sec:limit} is
