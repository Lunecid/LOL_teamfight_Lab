...
% AUC, match bootstrap, attribution forensics), R1 (results must not hinge on hand-set constants: pointer to
% the G x D sweep and the presence-gate points, both pending) and the audit-driven withdrawal that the
...
%   gxd_sweep, presence_gate_v33 (same keys as sec_definition.tex), shap_forensics, leak_ablation_ci.
...
Learners other than gradient-boosted trees are compared in Section~\fwdref{sec:learners}, and the label
...
patches. The \emph{patch holdout} does: models are trained on patch 15.14, stopped early on 15.15 and
tested once on 15.16 (Section~\fwdref{sec:learners-protocol}). \emph{Stopping early} means that trees
stop being added once the model's score on patch 15.15 has not improved for 100 consecutive trees, and
the model at its best point is kept; more trees would fit accidental regularities of the training patch
...
% Not yet available: REPO/scripts/run_shap_leak_forensics_v33.py (commit 342e74c).  REV/A7-shap-forensics/
%   shap_leak_forensics_v33_m20000.json (05:29) predates the re-run and is not cited.  The r2 re-run failed
%   (REV/queue_state/A7-shap-leak-forensics-v33-m20000-r2.done rc 1: "json exists; pass --overwrite or another
%   --tag").  The r3 job in REV/queue_wave5.json passes --tag rerun, so its output will be
%   REV/A7-shap-forensics/rerun.json (script l.810-813), not the path in its described_outputs.  No number
%   is cited until that file exists.  Method: docstring "Why SHAP, and only here" (Shapley value definition; mean |SHAP| as
%   global importance; TreeSHAP of Lundberg et al. 2020 via Booster.predict(pred_contrib=True), verified
...
(\pending{shap\_forensics}{20,000-match run: attribution share of the leak-affected columns in the leaky
and the audit-fixed build, with match-bootstrap intervals}). We do not use attributions to explain what
decides engagements.
...
With the cut at four, the scoreboard model's own AUC is 0.070 lower in picks than in teamfights (paired
interval 0.061 to 0.079), whereas the published full-input model's pick and teamfight AUCs cannot be
distinguished ($-0.0033$, paired interval $-0.0118$ to $+0.0054$). For the full-input model this
difference depends on the cut, as it does under cross-validation (Section~\ref{sec:pred-cut}): its pick
...
% G x D grid: REPO/scripts/run_gd_sweep_v33.py docstring: G in {10, 12, 13.7, 16, 18} s x D in {3,500, 4,000,
%   4,264, 4,750} u (20 settings), every other constant at preset v3.3, declared match subsample, published
%   LightGBM configuration under the patch holdout (lgbm_paper), per setting engagements, test AUC with match
%   bootstrap CI, class AUCs, and engagement overlap with the operating point.  No artefact on 2026-09-14.
% Presence gate: REPO/scripts/run_presence_gate_points.py docstring: points rule_R1600_B15 (1,600 u, 15 s),
%   ref_R1800_B10 (1,800 u, 10 s), rule_R1600_B15_M3 (M = 3); --engine holdout = the v3.3 protocol of the
%   G x D sweep.  The earlier runs_presence_gate/ comparison used the CoG constants and the leaking inputs and
%   is not cited (sec_definition.tex note).  No v3.3 artefact on 2026-09-14.
...
compared with the operating point on the same matches rather than with the cross-validated AUC above. The
first varies $G$ over 10, 12, 13.7, 16 and 18\,s and $D$ over 3,500, 4,000, 4,264 and 4,750\,u:
\pending{gxd\_sweep}{G x D sensitivity grid against the v3.3 operating point: engagements, test AUC with
match-bootstrap interval, class AUCs and overlap with the operating point's engagements for each of the 20
settings}. The second moves the presence gate to the conference point ($R = 1{,}800$\,u, $B = 10$\,s) and
raises the minimum number of champions per team from two to three:
\pending{presence\_gate\_v33}{v3.3 protocol re-run at (R, B, M) = (1,800 u, 10 s, 2) and (1,600 u, 15 s,
3), every other constant fixed: engagements, AUC at each point, and AUC on the engagements accepted at both
points}. Until both are available, no statement is made that the result is insensitive to these
constants.
