...
- **현재 숫자(v3.3, 공통 모집단 566,452행, 주 라벨 532,547)**: 전체 AUC **0.670**(market_event) /
  0.694(market_lex), 규모별 차이 없음(pick − teamfight −0.002). v2·v3의 teamfight 0.78~0.81과 0.1의
  규모 차이는 time_norm 누수였다(DEFINITION_EVIDENCE 22절).
...
|---|---|---|---|---|---|
| **market_event** (주 라벨, v3.3: 귀속·가격판) | 게임이 지급한 킬 골드(bounty + shutdownBounty + 20 + 45 × 어시스트) + 회귀 가격표(구조물·몬스터·와드), 교전 중심 D 안 사건만, ms 정확, 보간 없음 | ±300 g | 클러스터 킬 → 마지막 킬 시점 생존자 → 구조물·오브젝트 부호 | 제외 (6%) | 0.670 |
| market_lex (견고성 열) | 팀 골드(60 s 프레임 선형 보간)의 창 변화 차이 | ±300 g | 같음 | 제외 | 0.694 |
| Eq.3 attention_value_win (CoG, 부록) | 연구자 가중 사건 가치의 소프트맥스 합 | — | — | 동전 | 0.624 |
...
**코퍼스 v3 → v3.3** (v3.3 = 누수 제거 + 감사 대응; 22절)
...
**결정됨(v3.3)**: 주 라벨 = market_event 귀속·가격판(헤드라인 0.670, 규모 무관), market_lex·Eq.3은 견고성 열. 식별자는 숫자 입력 유지.
...
- **2026-09-09 v3.3 확정**: 외부 감사 8항목 대응 후 재구축. 주 라벨 market_event(귀속·가격) 0.670,
  market_lex 0.694, Eq.3 0.624; 규모 기울기 소멸. preset `v3.3`(`LOL_CFG_PRESET`)과 규칙 앵커 spec
  (`config/fight_boundary/`)으로 재현. 세부 표는 DEFINITION_EVIDENCE 22.1절.
